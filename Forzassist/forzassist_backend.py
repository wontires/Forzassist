...
YAW_FEEDFORWARD_WHEELBASE_M = 2.70
YAW_FEEDFORWARD_STEER_LOCK_DEG = 45.0
YAW_FEEDFORWARD_SPEED_FLOOR_MPS = 3.0
YAW_FEEDFORWARD_CENTER_WIDTH_FRACTION = 0.45
YAW_FEEDFORWARD_BALANCED_SLIP_ANGLE = 90.0
YAW_FEEDFORWARD_BALANCED_YAW_RATE = 1.7
...
def true_body_angle_deg(vx: float, vz: float) -> float:
...
def circular_angle_delta_deg(current_deg: float, previous_deg: float) -> float:

    delta = math.radians(current_deg - previous_deg)
...
def yaw_curvature_feedforward(desired_yaw: float, speed_mps: float) -> float:

    speed = max(abs(speed_mps), YAW_FEEDFORWARD_SPEED_FLOOR_MPS)

    road_wheel_deg = math.degrees(math.atan((YAW_FEEDFORWARD_WHEELBASE_M * desired_yaw) / speed))

    return road_wheel_deg / YAW_FEEDFORWARD_STEER_LOCK_DEG
...
class AssistState:
    ...
    def __init__(self):
        ...
        self.prev_true_angle: Optional[float] = None

        self.true_angle_unwrapped: float = 0.0

        self.spin_hook_active: bool = False

        self.spin_hook_dir: float = 0.0

        self.spin_hook_progress_dir: float = 0.0

        self.spin_hook_start_unwrapped: float = 0.0

        self.spin_hook_exit_unwrapped: float = 0.0

        self.spin_hook_total_progress: float = 0.0

        self.spin_handoff_active: bool = False

        self.spin_handoff_stick_dir: float = 0.0

        self.overlay_actual_steer_mode: bool = False

        self.reset_solver_debug_values()

    def reset_solver_debug_values(self) -> None:

        self.intent_max_slip_angle: float = 0.0

        self.intent_max_yaw_rate: float = 0.0

        self.slip_ff_gain: float = 0.0

        self.target_slip: float = 0.0

        self.slip_body: float = 0.0

        self.solver_slip_body: float = 0.0

        self.true_slip_body: float = 0.0

        self.true_angle_delta: float = 0.0

        self.angle_error: float = 0.0

        self.desired_yaw: float = 0.0

        self.yaw_rate: float = 0.0

        self.yaw_error: float = 0.0

        self.yaw_curvature_ff: float = 0.0

        self.yaw_ff_center_blend: float = 0.0

        self.yaw_ff_drive: float = 0.0

        self.yaw_ff_support_gate: float = 0.0

        self.yaw_ff_carry_yaw_rate: float = 0.0

        self.yaw_ff_carry_yaw: float = 0.0

        self.baseline: float = 0.0

        self.normal_raw: float = 0.0
...
class AssistBackend(QObject):
    ...
    def _detect_drift(self, tel: dict, st: AssistState) -> bool:

        def slip_ratio(field_name: str) -> float:

            try:

                value = abs(float(tel.get(field_name, 0.0)))
        ...
        vx = float(tel.get("VelocityX", 0.0))

        vz = float(tel.get("VelocityZ", 0.0))

        true_body_angle = abs(true_body_angle_deg(vx, vz))

        front_left = slip_ratio("TireSlipRatioFrontLeft")

        front_right = slip_ratio("TireSlipRatioFrontRight")

        rear_left = slip_ratio("TireSlipRatioRearLeft")

        rear_right = slip_ratio("TireSlipRatioRearRight")

        front_has_highest_slip = (

            (front_left > rear_left and front_left > rear_right)

            or (front_right > rear_left and front_right > rear_right)

        )

        rear_has_highest_slip = (

            (rear_left > front_left and rear_left > front_right)

            or (rear_right > front_left and rear_right > front_right)

        )

        high_angle_lock = true_body_angle >= 70.0
        ...
        if front_has_highest_slip:

            return False

        if rear_has_highest_slip:
    ...
    def _solve_drift_steering(
        ...
        true_slip_body = true_body_angle_deg(vx, vz)

        if st.prev_true_angle is None:

            st.prev_true_angle = true_slip_body

            st.true_angle_unwrapped = true_slip_body

            true_angle_delta = 0.0

        else:

            true_angle_delta = circular_angle_delta_deg(true_slip_body, st.prev_true_angle)

            st.true_angle_unwrapped += true_angle_delta

            st.prev_true_angle = true_slip_body

        st.intent_max_slip_angle = intent_max_slip_angle

        st.intent_max_yaw_rate = intent_max_yaw_rate

        st.slip_ff_gain = slip_ff_gain

        st.yaw_rate = yaw_rate

        st.slip_body = slip_body

        st.true_slip_body = true_slip_body

        st.true_angle_delta = true_angle_delta

        if tel.get("Gear", 1) == 0:

            st.spin_hook_active = False

            st.spin_hook_dir = 0.0

            st.spin_hook_progress_dir = 0.0

            st.spin_hook_start_unwrapped = 0.0

            st.spin_hook_exit_unwrapped = 0.0

            st.spin_hook_total_progress = 0.0

            st.spin_handoff_active = False

            st.spin_handoff_stick_dir = 0.0

            st.reverse_gear_active = True

            st.reverse_gear_ff_mult = 0.0

            st.target_slip = stick_x * intent_max_slip_angle

            st.solver_slip_body = slip_body

            st.angle_error = st.target_slip - st.solver_slip_body

            st.desired_yaw = 0.0

            st.yaw_error = 0.0

            st.yaw_curvature_ff = 0.0

            st.yaw_ff_center_blend = 0.0

            st.yaw_ff_drive = 0.0

            st.yaw_ff_support_gate = 0.0

            st.yaw_ff_carry_yaw_rate = 0.0

            st.yaw_ff_carry_yaw = 0.0

            st.baseline = 0.0

            st.normal_raw = stick_x

            st.raw_steer_target = stick_x

            st.steer_saturated = abs(stick_x) >= FORZA_FULL_LOCK_STICK

            return clamp(stick_x, -FORZA_FULL_LOCK_STICK, FORZA_FULL_LOCK_STICK)
        ...
        solver_slip_body = slip_body

        angle_error = target_slip - solver_slip_body

        desired_yaw = angle_error / intent_max_slip_angle * intent_max_yaw_rate

        yaw_error = (desired_yaw - yaw_rate) / intent_max_yaw_rate

        baseline = solver_slip_body * slip_ff_gain * ff_mult

        speed_mps = float(tel.get("Speed", math.sqrt(vx * vx + vz * vz)))

        center_width = max(
            max(intent_max_slip_angle, YAW_FEEDFORWARD_BALANCED_SLIP_ANGLE)
            * YAW_FEEDFORWARD_CENTER_WIDTH_FRACTION,
            0.001,
        )

        yaw_ff_center_blend = 1.0 - smoothstep01(abs(solver_slip_body) / center_width)

        slip_error_pressure = smoothstep01(abs(angle_error) / max(intent_max_slip_angle, 0.001))

        yaw_ff_support_gate = yaw_ff_center_blend * slip_error_pressure

        slip_command = angle_error / max(intent_max_slip_angle, 0.001)

        yaw_ff_carry_yaw_rate = intent_max_yaw_rate + (
            max(YAW_FEEDFORWARD_BALANCED_YAW_RATE - intent_max_yaw_rate, 0.0)
            * yaw_ff_support_gate
        )

        yaw_ff_carry_yaw = slip_command * yaw_ff_carry_yaw_rate

        yaw_curvature_ff = yaw_curvature_feedforward(yaw_ff_carry_yaw, speed_mps)

        yaw_ff_drive = yaw_curvature_ff * yaw_ff_support_gate * ff_mult

        normal_raw = baseline + yaw_error + yaw_ff_drive

        raw_target = normal_raw

        target = clamp(raw_target, -FORZA_FULL_LOCK_STICK, FORZA_FULL_LOCK_STICK)

        st.target_slip = target_slip

        st.solver_slip_body = solver_slip_body

        st.angle_error = angle_error

        st.desired_yaw = desired_yaw

        st.yaw_error = yaw_error

        st.yaw_curvature_ff = yaw_curvature_ff

        st.yaw_ff_center_blend = yaw_ff_center_blend

        st.yaw_ff_drive = yaw_ff_drive

        st.yaw_ff_support_gate = yaw_ff_support_gate

        st.yaw_ff_carry_yaw_rate = yaw_ff_carry_yaw_rate

        st.yaw_ff_carry_yaw = yaw_ff_carry_yaw

        st.baseline = baseline

        st.normal_raw = normal_raw
    ...
    def _compute_steering(self, tel: dict, stick_x: float, st: AssistState, cfg: AssistConfig) -> float:
        ...
        is_drifting = self._detect_drift(tel, st)
        ...
        if not is_drifting:
            ...
            st.reset_solver_debug_values()

            st.intent_max_slip_angle = intent_angle

            st.intent_max_yaw_rate = yaw_rate

            st.slip_ff_gain = ff_gain

            st.slip_body = body_slip_deg(vx, vz)

            st.true_slip_body = true_body_angle_deg(vx, vz)
        ...
        drift_target = self._solve_drift_steering(

            tel, stick_x, st, intent_angle, yaw_rate, ff_gain

        )

        final_target = clamp(drift_target, -1.0, 1.0)
        ...
        try:
            ...
            while not self._stop_event.is_set():
                ...
                if race_active:
                    ...
                    self._state.prev_true_angle = None

                    self._state.true_angle_unwrapped = 0.0

                    self._state.spin_hook_active = False

                    self._state.spin_hook_dir = 0.0

                    self._state.spin_hook_progress_dir = 0.0

                    self._state.spin_hook_start_unwrapped = 0.0

                    self._state.spin_hook_exit_unwrapped = 0.0

                    self._state.spin_hook_total_progress = 0.0

                    self._state.spin_handoff_active = False

                    self._state.spin_handoff_stick_dir = 0.0

                    self._state.overlay_actual_steer_mode = False

                    self._state.reset_solver_debug_values()
                ...
                if now - last_overlay_emit >= overlay_interval:
                    ...
                    if race_active:
                        ...
                        assist_overlay_delta = final_steer_for_overlay - stick_x

                        overlay_body_angle = abs(true_body_angle_deg(

                            float(tel.get("VelocityX", 0.0)),

                            float(tel.get("VelocityZ", 0.0)),
                        ...
                        opposing_overlay = (
                            ...
                            and abs(final_steer_for_overlay) > 0.02

                            and sign(stick_x) != sign(final_steer_for_overlay)

                        )

                        if self._state.is_drifting and (overlay_body_angle >= 20.0 or opposing_overlay):

                            self._state.overlay_actual_steer_mode = True

                        elif (not self._state.is_drifting) or (overlay_body_angle < 8.0 and abs(assist_overlay_delta) < 0.04):

                            self._state.overlay_actual_steer_mode = False

                        if self._state.overlay_actual_steer_mode:

                            assist_overlay_steer = final_steer_for_overlay

                        else:

                            assist_overlay_steer = assist_overlay_delta

                    else:

                        self._state.overlay_actual_steer_mode = False

                        assist_overlay_steer = 0.0
